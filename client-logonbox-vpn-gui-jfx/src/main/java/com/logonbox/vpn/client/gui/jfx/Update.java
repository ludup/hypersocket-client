...
public class Update extends AbstractController {
	...
	public void updateProgressed(String app, long sincelastProgress, long totalSoFar, long totalBytesExpected) {
		String appName = getAppName(app);
		this.message.textProperty().set(MessageFormat.format(resources.getString("updating"), appName));
		progress.progressProperty().setValue((double) totalSoFar / totalBytesExpected);
	}
	...
	public void bridgeEstablished() {
		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				if (awaitingBridgeEstablish != null) {
					/*
					 * If the connection that originated the update was known, then try to reconnect
					 * to it when the client GUI starts again
					 */
					if (updatingConnection != null) {
						LOG.info(String.format("Will use connection %d (%s) when next starting",
								updatingConnection.getId(), updatingConnection.getHostname()));
						Configuration.getDefault().temporaryOnStartConnectionProperty()
								.set(String.valueOf(updatingConnection.getId()));
					} else
						LOG.info(String.format(
								"Updating connection is not known, might not start connected to anything (unless some profiles are 'stay connected')",
								updatingConnection.getId(), updatingConnection.getHostname()));

					// Bridge established as result of update, now restart the
					// client itself
					resetAwaingBridgeEstablish();
					message.textProperty().set(resources.getString("guiRestart"));
		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				if (awaitingBridgeLoss != null) {
					...
					message.textProperty().set(resources.getString("waitingStart"));
	...
	private void resetState() {
		...
		// TODO
//		UI.getInstance().setMode(currentMode);
	}
	...
	private void giveUpWaitingForBridgeEstablish() {
		...
		// TODO
//		UI.getInstance().setMode(UIState.IDLE);
	}

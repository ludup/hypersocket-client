...
public abstract class AbstractClientServiceImpl<C extends ClientContext<L>, J extends AbstractConnectionJob<?>, L extends LocalContext<? extends ClientService>>
	...
	public AbstractClientServiceImpl(L context) {
	...
	public void scheduleConnect(Connection c) throws RemoteException {
		...
		Integer reconnectSeconds = new Integer(configurationService.getValue("client.reconnectInSeconds", "5"));
	...
	protected abstract J createJob(Connection c) throws RemoteException;
		...
		try {
			...
			while (urls.hasMoreElements()) {
				...
				try {
					...
					Class<ServicePlugin<C>> pluginClz = (Class<ServicePlugin<C>>) Class.forName(clz);

					ServicePlugin<C> plugin = pluginClz.newInstance();
	...
	protected abstract C createClientContent(final HypersocketClient<Connection> client);
	...
	public void finishedConnecting(Connection connection, J job) {
		synchronized (activeClients) {
